...
interface GoogleMapSimpleProps {
  ...
  showWaterways?: boolean; // New optional prop with default value of false
...
const GoogleMapSimple: React.FC<GoogleMapSimpleProps> = ({
  ...
  onMapClick,
  isAddingPinpoint,
  showWaterways = false, // Default to false
  ...
  const initializeMap = useCallback(() => {
    ...
    try {
      ...
      clickListenerRef.current = mapWithListener.addListener(
        "click",
      ...
      // Render waterways
      if (window.google?.maps && showWaterways) {
        const google = window.google;
        waterways.forEach((waterway) => {
          const path = waterway.coordinates.map((coord) => ({
            lat: coord.lat,
            lng: coord.lng,
          }));
          const color =
          ...
          const polylineWithListener = polyline as {
            addListener: (event: string, callback: () => void) => void;
          };
          polylineWithListener.addListener("click", () =>
      ...
      if (window.google?.maps) {
        ...
        pinpoints.forEach((pinpoint) => {
          ...
          if (
            pinpoint.lastActionSummary === "CLEANING" ||
            pinpoint.lastActionSummary === "BOTH"
          ) {
          ...
          if (
            ...
            const markerWithListener = marker as {
              addListener: (event: string, callback: () => void) => void;
            };
            markerWithListener.addListener("click", () =>
  ...
  useEffect(() => {
    if (mapInstanceRef.current && window.google?.maps) {
      ...
      const mapWithListener = mapInstanceRef.current as {
        addListener: (
          event: string,
          callback: (event: {
            latLng: { lat: () => number; lng: () => number };
  ...
  useEffect(() => {
    ...
    if (
      ...
      console.log(
        ...
        "Length:",
        GOOGLE_MAPS_CONFIG.API_KEY.length
      );
