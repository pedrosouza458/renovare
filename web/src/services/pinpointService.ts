...
const mapPinpointFromBackend = (backendPinpoint: BackendPinpoint): Pinpoint => {
  return {
    ...backendPinpoint,
    posts:
      backendPinpoint.posts?.map((post: BackendPost) => ({
        ...post,
...
export const pinpointService = {
  async getAllPinpoints(): Promise<Pinpoint[]> {
    const backendPinpoints =
      (await pinService.getAllPins()) as BackendPinpoint[];
  ...
  async createPinpointWithPost(
    ...
    const createPostPayload: CreatePostData = {
      type: mapPostTypeToBackend(postData.type) as
        | "ALERT"
        | "CLEANING"
        | "BOTH",
      text: postData.text,
    ...
    try {
      const score = await calculatePostScore(
        postData.type,
        latitude,
        longitude
      );
      ...
      console.log(
        `User score updated: +${score} points for ${postData.type} post`
      );
    } catch (error) {
  ...
  async addPostToPinpoint(
    ...
    postData: {
      type: PostType;
      text: string;
      photos?: Array<{ url: string; isBefore?: boolean }>;
    }
  ): Promise<Pinpoint> {
    // Get the pinpoint first to access its coordinates for score calculation
    const pinpoint = (await pinService.getPinById(
      pinpointId
    )) as BackendPinpoint;
    ...
    const updatedPinpoint = (await pinService.getPinById(
      pinpointId
    )) as BackendPinpoint;
