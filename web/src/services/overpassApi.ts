...
class OverpassApiService {
  ...
  async getWaterways(params: OverpassQueryParams): Promise<WaterwayData[]> {
    try {
      ...
      throw new OverpassApiError(
        `Failed to fetch waterways: ${error instanceof Error ? error.message : 'Unknown error'}`
      );
    }
  }
  ...
  async searchWaterwaysByName(searchTerm: string, params: OverpassQueryParams): Promise<WaterwayData[]> {
    try {
      ...
      throw new OverpassApiError(
        `Failed to search waterways: ${error instanceof Error ? error.message : 'Unknown error'}`
      );
    }
  }
  ...
  async getMajorWaterways(params: OverpassQueryParams): Promise<WaterwayData[]> {
    try {
      ...
      throw new OverpassApiError(
        `Failed to fetch major waterways: ${error instanceof Error ? error.message : 'Unknown error'}`
      );
    }
  }
}
